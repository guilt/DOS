...
/* Category variables.  */
short cCat;
char __far *aCat[MAXVALUES];

/* Series variables.  */
short cSeries;
short __far acSeries[MAXSERIES];
char __far *aSeriesName[MAXSERIES];
 ...
 */
float __far aValue[MAXSERIES][MAXVALUES];
 ...
 */
float __far axValue[MAXVALUES];
float __far ayValue[MAXVALUES];

/* Data values for multi-series scatter charts.  */
float __far axValueMS[MAXVALUES][MAXVALUES];
float __far ayValueMS[MAXVALUES][MAXVALUES];

/* Exploded flags for pie chart.  */
short __far aExplode[MAXVALUES];
...
void ShowSampleData()
{
    ...
    for( iSeries = 1; iSeries <= 3; iSeries++ )
    {
        ...
        for( iValue = 1; iValue <= 12; iValue++ )
        {
            ...
            PrintAt( y, iValue * 6, (char __far *)szTmp, -1 );
        }
    }
    ...
    switch( ce.charttype )
    {
        ...
        case _PG_LINECHART:
            ...
            /* If multiple-series, then data is OK. Just draw chart.  */
                ...
                iErr = _pg_chartms( &ce, aCat, (float __far *)aValue,
        ...
        case _PG_SCATTERCHART:
            ...
            else
            {
                ...
                iErr = _pg_chartscatterms( &ce, (float __far *)axValueMS,
                                           (float __far *)ayValueMS,
